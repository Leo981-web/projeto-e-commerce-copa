...
export function mapProduct(product) {
  ...
  const finalPrice = (product.convertedPrice && product.convertedPrice > 0) 
    ? product.convertedPrice 
    : product.price;

  return {
    id: product.id,
    name: `${product.brand || ''} ${product.model || ''}`.trim() || product.description,
...
function mapToDTO(productData) {
  const rawStock = productData.quantity !== undefined ? productData.quantity : productData.stock;
...
export async function getProductById(id) {
  try {
    ...
    const response = await fetch(`${API_URL}/products/${id}?targetCurrency=BRL`, { 
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    });
...
export async function deleteProduct(id) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorText.includes("Unexpected row count") || errorText.includes("expected row count 1 but was 0")) {
        console.warn(`Aviso: O produto ID ${id} já não existia no banco de dados.`);
        return true;
      }
