...
export default function ProfileScreen({ navigation }) {
  ...
  async function handleSaveField() {
    ...
    try {
      if (editModal.field === "name") {
        ...
        showAlert({ title: t("successTitle"), message: t("updateNameSuccess"), type: "success" });
      } else if (editModal.field === "email") {
        await authService.updateProfileEmail(editModal.value);
        showAlert({ title: t("successTitle"), message: t("updateEmailSuccess"), type: "success" });
      } else {
        await authService.updateProfilePhone(editModal.value);
        showAlert({ title: t("successTitle"), message: t("updatePhoneSuccess"), type: "success" });
      }
      await refreshUser();
      closeEditModal();
    } catch (error) {
      showAlert({ title: t("profileUpdateError"), message: error.message, type: "danger" });
    } finally {
  ...
  async function handleChangePassword() {
    try {
      ...
      showAlert({ title: t("successTitle"), message: t("passwordResetSent"), type: "success" });
    } catch (error) {
      showAlert({ title: t("profileUpdateError"), message: error.message, type: "danger" });
    }
  }
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.bg }]}>
      ...
      >
        {/* ── CARD DE AVATAR ── */}
        <View style={[styles.avatarCard, { backgroundColor: theme.card, shadowColor: isDarkMode ? "#000" : "#a39f96" }]}>
          <View style={[styles.avatarPlaceholder, { backgroundColor: NAVY_BLUE }]}>
            <Text style={styles.avatarInitials}>{initials}</Text>
          </View>
          <Text style={[styles.avatarName, { color: theme.textPrimary }]}>{t("hello")}, {user?.name}!</Text>
          <Text style={[styles.avatarEmail, { color: theme.textMuted }]}>{user?.email}</Text>
        </View>

        {/* ── DADOS PESSOAIS ── */}
        <Text style={[styles.sectionTitle, { color: theme.textMuted }]}>{t("personalData")}</Text>
        <View style={[styles.optionsBlock, { backgroundColor: theme.card, shadowColor: isDarkMode ? "#000" : "#a39f96" }]}>
          ...
          >
            <View style={[styles.iconContainer, { backgroundColor: theme.iconBg }]}>
              <MaterialIcons name="person" size={22} color={theme.textPrimary} />
            </View>
            <View style={styles.settingTextContainer}>
              <Text style={[styles.settingLabel, { color: theme.textMuted }]}>{t("profileName")}</Text>
              <Text style={[styles.settingValue, { color: theme.textPrimary }]} numberOfLines={1}>{user?.name}</Text>
            </View>
          ...
          >
            ...
            <View style={styles.settingTextContainer}>
              <Text style={[styles.settingLabel, { color: theme.textMuted }]}>{t("profileEmail")}</Text>
              <Text style={[styles.settingValue, { color: theme.textPrimary }]} numberOfLines={1}>{user?.email}</Text>
            </View>
            <MaterialIcons name="edit" size={20} color={theme.textMuted} />
          </TouchableOpacity>
          ...
          >
            ...
            <View style={styles.settingTextContainer}>
              <Text style={[styles.settingLabel, { color: theme.textMuted }]}>{t("profilePhone")}</Text>
        ...
        {/* ── SEGURANÇA ── */}
        <Text style={[styles.sectionTitle, { color: theme.textMuted }]}>{t("security")}</Text>
        <View style={[styles.optionsBlock, { backgroundColor: theme.card, shadowColor: isDarkMode ? "#000" : "#a39f96" }]}>
          <TouchableOpacity
            style={[styles.settingRow, styles.settingRowLast]}
            activeOpacity={0.7}
          ...
          >
            ...
            <View style={styles.settingTextContainer}>
              <Text style={[styles.settingTitle, { color: theme.textPrimary }]}>{t("changePassword")}</Text>
              <Text style={[styles.settingSubtitle, { color: theme.textMuted }]}>{t("changePasswordSub")}</Text>
            </View>
        ...
        <Text style={[styles.sectionTitle, { color: theme.textMuted }]}>{t("purchaseHistory")}</Text>
        <View style={[styles.optionsBlock, { backgroundColor: theme.card, shadowColor: isDarkMode ? "#000" : "#a39f96" }]}>
          <View style={styles.emptyState}>
            <MaterialIcons name="receipt-long" size={36} color={theme.textMuted} />
            <Text style={[styles.emptyText, { color: theme.textMuted }]}>{t("noPurchases")}</Text>
          </View>
        </View>
        ...
        <View style={[styles.optionsBlock, { backgroundColor: theme.card, shadowColor: isDarkMode ? "#000" : "#a39f96" }]}>
          ...
          >
            ...
            <View style={styles.settingTextContainer}>
              <Text style={[styles.settingTitle, { color: "#b42318" }]}>{t("logoutProfile")}</Text>
              <Text style={[styles.settingSubtitle, { color: theme.textMuted }]}>{t("logoutProfileSub")}</Text>
            </View>
          ...
          >
            {tab.center ? (
              ...
              <>
                ...
                />
                <Text style={[
                  styles.navLabel, { color: theme.navInactive },
                  activeNav === tab.key && { color: theme.navActive, fontWeight: "700" },
                ]}>
                  {t(tab.labelKey)}
                </Text>
                {activeNav === tab.key && (
                  <View style={[styles.navDot, { backgroundColor: theme.navActive }]} />
                )}
              </>
            )}
          </TouchableOpacity>
      ...
      {/* ── MODAL DE EDIÇÃO ── */}
      <Modal visible={editModal.visible} transparent animationType="fade" onRequestClose={closeEditModal}>
        <Pressable style={styles.modalOverlay} onPress={closeEditModal}>
          <Pressable style={[styles.modalCard, { backgroundColor: theme.card }]} onPress={() => {}}>
            <Text style={[styles.modalTitle, { color: theme.textPrimary }]}>
              {editModal.field === "name" ? t("editName") :
               editModal.field === "email" ? t("editEmail") : t("editPhone")}
            </Text>
            <AppInput
              label={
                editModal.field === "name" ? t("newNameLabel") :
                editModal.field === "email" ? t("newEmailLabel") : t("newPhoneLabel")
              }
              icon={
                editModal.field === "name" ? "person" :
                editModal.field === "email" ? "email" : "phone"
              }
              placeholder={
                editModal.field === "name" ? t("namePlaceholder") :
                editModal.field === "email" ? t("emailPlaceholder") : t("phonePlaceholder")
              }
            ...
            />
            <AppButton title={t("cancel")} variant="ghost" onPress={closeEditModal} />
          </Pressable>
        </Pressable>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 13, fontWeight: "500", marginBottom: 4, marginLeft: 2 },
  headerTitleRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  backButton: { justifyContent: "center", alignItems: "center", paddingVertical: 4, paddingRight: 4 },
  headerTitle: { fontSize: 20, fontWeight: "900", letterSpacing: 3 },

  scrollContent: { paddingHorizontal: 20, paddingBottom: 130 },

  avatarCard: {
    borderRadius: 18, paddingVertical: 24, paddingHorizontal: 16,
  ...
  sectionTitle: {
    ...
    letterSpacing: 1.2, marginBottom: 8, marginLeft: 4, marginTop: 4,
  },
  optionsBlock: {
  ...
  settingRow: {
    ...
    paddingVertical: 14, paddingHorizontal: 16, borderBottomWidth: 1.5,
  },
  settingRowLast: {
    flexDirection: "row", alignItems: "center",
    paddingVertical: 14, paddingHorizontal: 16, borderBottomWidth: 0,
  },
  ...
  bottomNav: {
    ...
    flexDirection: "row", paddingTop: 10, paddingBottom: 24, paddingHorizontal: 10,
    borderTopLeftRadius: 24, borderTopRightRadius: 24,
    shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.08, shadowRadius: 16, elevation: 10,
  },
  navItem: { flex: 1, alignItems: "center", justifyContent: "center", gap: 2 },
  navItemCenter: { marginTop: -28 },
  navCreateBtn: {
    width: 54, height: 54, borderRadius: 27, alignItems: "center", justifyContent: "center",
    shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 8, borderWidth: 4,
  },
