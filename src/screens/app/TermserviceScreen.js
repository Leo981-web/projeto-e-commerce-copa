...
import { useLanguage } from "../../context/LanguageContext"; // Importação adicionada
...
function TermSection({ item, index, theme }) {
  ...
  return (
    <View style={[sec.wrap, { backgroundColor: theme.card, borderColor: theme.divider }]}>
      <TouchableOpacity
        style={sec.header}
        activeOpacity={0.7}
        onPress={() => setOpen((p) => !p)}
...
const sec = StyleSheet.create({
  wrap: { borderRadius: 14, marginBottom: 10, overflow: "hidden", borderWidth: 1.5, elevation: 2 },
  header: { flexDirection: "row", alignItems: "center", paddingHorizontal: 14, paddingVertical: 14, gap: 12 },
  ...
  body: { flexDirection: "row", paddingHorizontal: 14, paddingBottom: 14, gap: 10 },
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.bg }]}>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroTop}>
          ...
          <View style={styles.heroTexts}>
            <Text style={styles.heroTitle}>
              {/* Ajuste: Aplicado o t() flexível com fallback estático nativo */}
        ...
        <View style={styles.heroChips}>
          <View style={styles.chip}>
            ...
            <Text style={styles.chipText}>{t("termsEffectiveDate") || "Vigência: Jan 2026"}</Text>
          </View>
          <View style={styles.chip}>
            <MaterialIcons name="verified-user" size={11} color="rgba(255,255,255,0.75)" />
            <Text style={styles.chipText}>{t("termsLgpdCompliance") || "LGPD Conformidade"}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.summaryBar}>
        ...
        <Text style={styles.summaryText}>
          {t("termsSummaryInstruction") || "Toque em cada seção para expandir e ler os detalhes."}
        </Text>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.footer}>
          ...
          <View style={[styles.contactCard, { backgroundColor: theme.card, borderColor: theme.divider }]}>
            <MaterialIcons name="mail-outline" size={20} color={GREEN} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.contactTitle, { color: theme.textPrimary }]}>{t("termsPrivacyQuestion") || "Dúvidas sobre privacidade?"}</Text>
...
const styles = StyleSheet.create({
  safe: { flex: 1 },
  hero: { backgroundColor: GREEN_DARK, paddingTop: 16, paddingBottom: 20, paddingHorizontal: 20, position: "relative", overflow: "hidden" },
  stripe1: { position: "absolute", top: 0, bottom: 0, right: 60, width: 1, backgroundColor: "rgba(255,255,255,0.06)" },
  stripe2: { position: "absolute", top: 0, bottom: 0, right: 120, width: 1, backgroundColor: "rgba(255,255,255,0.04)" },
  heroTop: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 14 },
  backBtn: { width: 38, height: 38, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.13)", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  heroTexts: { flex: 1 },
  heroTitle: { fontSize: 26, fontWeight: "900", color: WHITE, letterSpacing: -0.5, lineHeight: 28 },
  heroChips: { flexDirection: "row", gap: 8 },
  chip: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: "rgba(255,255,255,0.10)", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999 },
  chipText: { fontSize: 11, fontWeight: "600", color: "rgba(255,255,255,0.80)" },
  summaryBar: { flexDirection: "row", alignItems: "center", gap: 8, marginHorizontal: 20, marginTop: 16, marginBottom: 12, paddingHorizontal: 14, paddingVertical: 10, backgroundColor: "rgba(21,98,42,0.07)", borderRadius: 10, borderWidth: 1.5, borderColor: "rgba(21,98,42,0.15)" },
  summaryText: { flex: 1, fontSize: 12, color: GREEN, fontWeight: "600", lineHeight: 16 },
  ...
  contactCard: { flexDirection: "row", alignItems: "center", gap: 12, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, width: "100%", borderWidth: 1.5 },
