...
export default function AboutScreen({ navigation }) {
  ...
  async function openLinkedin(url) {
    ...
    catch (error) { showAlert({ title: t("opsTitle"), message: error.message, type: "danger" }); }
  }
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: screenBg }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.sectionHeader}>
          ...
          <View>
            <Text style={[styles.sectionTitle, { color: GREEN }]}>{t("aboutTeamTitle")}</Text>
            <Text style={[styles.sectionSub, { color: mutedColor }]}>{t("aboutTeamSub")}</Text>
          </View>
        </View>

        <View style={[styles.teamCard, { backgroundColor: cardBg, borderColor: dividerColor }]}>
            ...
            return (
              ...
              >
                <View style={[styles.avatar, { backgroundColor: palette.bg }]}>
                  <Text style={[styles.avatarText, { color: palette.text }]}>{initials(member.name)}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  heroAppName: { fontSize: 28, fontWeight: "900", letterSpacing: -0.5, lineHeight: 30 },
  heroTagline: { fontSize: 12, fontWeight: "600", marginTop: 2 },
  heroDivider: { height: 1, marginHorizontal: 20, marginBottom: 14 },
  heroDescription: { fontSize: 13, lineHeight: 20, paddingHorizontal: 20, marginBottom: 16 },
